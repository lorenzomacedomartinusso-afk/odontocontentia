...
const LandingPage: React.FC<{ onLogin: () => void }> = ({ onLogin }) => (
  <div className="min-h-screen flex flex-col relative overflow-hidden bg-zinc-950">
    ...
    <main className="flex-1 flex flex-col w-full z-10 relative">
      ...
      {/* SECTION 8: Pricing Table - Premium Card */}
      <section className="py-32 px-4 relative">
        {/* Background accent */}
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-brand-teal/5 rounded-full blur-[150px] pointer-events-none" />

        <div className="max-w-5xl mx-auto text-center relative z-10">
          <Reveal>
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-16">Comece hoje — plano acessível para todo dentista</h2>
          </Reveal>
          ...
          <Reveal delay={200} direction="up">
            <div className="max-w-lg mx-auto relative">
              ...
              <div className="absolute -top-5 left-1/2 -translate-x-1/2 z-20">
                <div className="bg-gradient-to-r from-brand-teal to-emerald-500 text-brand-black px-6 py-2 rounded-full text-xs font-bold uppercase tracking-widest shadow-xl shadow-brand-teal/30 animate-bounce-subtle flex items-center gap-2">
              ...
              <div className="relative group">
                ...
                <div className="absolute -inset-[2px] bg-gradient-to-r from-brand-teal via-emerald-400 to-brand-teal rounded-3xl opacity-50 group-hover:opacity-80 blur-sm transition-opacity duration-500" style={{ backgroundSize: '200% 100%', animation: 'gradient-shift 4s ease infinite' }} />

                {/* Card content */}
                <div className="relative bg-zinc-950 rounded-3xl p-10 md:p-14 border border-zinc-800/50">

                  {/* Plan name */}
                  <p className="text-zinc-400 font-medium mb-4">Plano Pro Odonto</p>

                  {/* Price */}
                  <div className="flex items-baseline justify-center gap-2 mb-8">
                    <span className="text-6xl md:text-7xl font-bold bg-gradient-to-r from-white to-zinc-300 bg-clip-text text-transparent">R$ 34,90</span>
                    <span className="text-zinc-500 text-lg font-medium">/mês</span>
                  </div>
                  ...
                  <div className="space-y-5 mb-10">
                    {[
                      { icon: Sparkles, text: "Gerador Ilimitado de Roteiros e Legendas" },
                      { icon: LayoutDashboard, text: "Calendário Editorial & Kanban" },
                      { icon: Users, text: "Gestão de Equipe (até 3 usuários)" },
                      { icon: ShieldCheck, text: "IA treinada com normas do CFO" },
                      { icon: MessageCircle, text: "Suporte prioritário via WhatsApp" }
                    ].map((feat, i) => (
                      <div key={i} className="flex items-center gap-4 text-left group/feat">
                        <div className="w-10 h-10 rounded-xl bg-brand-teal/10 border border-brand-teal/20 flex items-center justify-center shrink-0 group-hover/feat:bg-brand-teal/20 group-hover/feat:border-brand-teal/40 transition-all duration-300">
                          <feat.icon className="w-5 h-5 text-brand-teal" />
                        </div>
                        <span className="text-zinc-300 font-medium group-hover/feat:text-white transition-colors">{feat.text}</span>
                      </div>
                    ))}
                  </div>

                  {/* CTA Button */}
                  <Button onClick={onLogin} variant="gradient" className="w-full py-6 text-lg font-bold shadow-[0_0_50px_-12px_rgba(45,212,191,0.5)] hover:shadow-[0_0_60px_-10px_rgba(45,212,191,0.7)] hover:scale-[1.02] transition-all duration-300">
